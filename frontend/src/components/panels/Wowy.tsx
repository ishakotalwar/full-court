...
import { useQueryIds, useQueryState } from "@/lib/url";
...
export function Wowy({ meta }: { meta: Meta }) {
  ...
  const [season, setSeason] = useQueryState("season", seasons.at(-1) ?? "");
  const [team, setTeam] = useQueryState("team");
  const [picked, setPicked] = useQueryIds("players");
  const [data, setData] = useState<any>(null);
  const [err, setErr] = useState<string | null>(null);

  // A new team means a new roster, so nobody stays selected across the change
  // — but only when it actually changes. Clearing on mount too would throw
  // away the players a link arrived carrying.
  const previous = useRef(`${team}|${season}|${meta.league}`);
  useEffect(() => {
    const now = `${team}|${season}|${meta.league}`;
    if (previous.current === now) return;
